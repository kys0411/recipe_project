...
public class UserUpdateController {
    ...
    public void updateButtonOnAction(ActionEvent event) {
        ...
        if (nickname.isEmpty() || password.isEmpty() || confirmPassword.isEmpty()) {
            showAlert(Alert.AlertType.ERROR, "회원수정 실패", "모든 필드를 입력해주세요.");
            return;
        }

        if (!password.equals(confirmPassword)) {
            showAlert(Alert.AlertType.ERROR, "회원수정 실패", "비밀번호가 일치하지 않습니다.");
            return;
        }
        ...
        try {
            userService.update(UserSession.getInstance().getLoggedUser().getId(), updateRequestDto);
            showAlert(Alert.AlertType.INFORMATION, "회원수정 성공", "회원수정 성공");
            ...
            showAlert(Alert.AlertType.ERROR, "회원수정 실패", "데이터베이스 오류가 발생했습니다.");
            e.printStackTrace();
        } catch (IOException e) {
